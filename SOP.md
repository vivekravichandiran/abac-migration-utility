...
| `ROW_FILTER` step `FAILED` with `error_code=RLS_TAG_COLLISION_UNRESOLVABLE` | 2+ columns of that table need the identical bare row-filter tag key (e.g. a row filter function with 2+ `USING COLUMNS` from the same table) — this tool never mints a disambiguating tag value, so it skips the tag assignment instead | manual remediation required for that table (e.g. redesign the row filter so it only ever needs one tag-bearing column per table); everything else in the run is unaffected — column masks on the same table are not subject to this restriction and still succeed |
| `INVENTORY` reports `NOT_ELIGIBLE`/`UNSUPPORTED_TABLE_TYPE` for a `MATERIALIZED_VIEW` or plain `VIEW` | those table types aren't supported yet — confirmed live that a materialized view rejects this tool's plain `ALTER TABLE ...` DDL outright (`EXPECT_TABLE_NOT_VIEW.NO_ALTERNATIVE`) | not yet supported — tracked in `DESIGN.md` §16 item 2; `STREAMING_TABLE` **is** supported (confirmed live end-to-end) so this only affects views/materialized views |

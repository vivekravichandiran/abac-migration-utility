...
| `INVENTORY` reports `NOT_ELIGIBLE`/`UNSUPPORTED_TABLE_TYPE` for a plain `VIEW` | views have no underlying storage of their own to attach a row filter/mask to | expected/permanent — not a bug; `MANAGED`/`EXTERNAL`/`STREAMING_TABLE`/`MATERIALIZED_VIEW` are all supported (confirmed live end-to-end for every one, see `DESIGN.md` §16 item 2) |

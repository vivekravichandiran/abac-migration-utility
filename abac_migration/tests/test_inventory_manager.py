...
def test_materialized_view_is_still_not_eligible():
    ...
    uc.set_row_filter_state(table, "cat.schema.rf_fn", ["region"])
    uc.tables[table.full_name] = "MATERIALIZED_VIEW"
...
def test_streaming_table_is_eligible():
    """STREAMING_TABLE was added to SUPPORTED_TABLE_TYPES after live
    testing (2026-09-15, ril_full_access_test.streaming_test) confirmed a
    full INVENTORY -> APPLY_ABAC -> FINALIZE cycle works unmodified against
    a real streaming table - plain `ALTER TABLE ...` DDL works fine, no
    `ALTER STREAMING TABLE` keyword needed (unlike MATERIALIZED_VIEW,
    see test_materialized_view_is_still_not_eligible)."""
    uc = FakeUnityCatalogGateway()
    table = TableRef("cat", "schema", "a_streaming_tbl")
    uc.set_row_filter_state(table, "cat.schema.rf_fn", ["region"])
    uc.tables[table.full_name] = "STREAMING_TABLE"

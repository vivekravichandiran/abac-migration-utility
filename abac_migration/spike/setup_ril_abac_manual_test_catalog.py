"""Second, independent test catalog on the TARGET workspace
(adb-7405609958717235.15), mirroring `setup_ril_abac_test_catalog.py`'s
structure exactly (same 4 domains, same RLS-only/mask-only/both split, same
`governance.group_membership` workaround) but with its own catalog name and
its own dummy data - kept fully separate from `ril_abac_test` so it can be
migrated/reset/manually poked at independently for ad-hoc manual testing
without disturbing the other catalog's state.

Catalog: ril_abac_manual_test
...
rationale on the group-membership control-table workaround and the
deliberate function-reuse-within-a-domain pattern (both apply identically
here).
"""
...
# Same workspace-local groups as ril_abac_test's setup - reused deliberately
# (they're domain-scoped identity, not catalog-scoped) rather than minting
# duplicates; create_group() is idempotent and no-ops if already present.
DOMAIN_GROUPS = {
    "hr": "hr_admins",
